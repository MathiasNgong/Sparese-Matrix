...
public:
	...
	int getElement(int currRow, int currCol){
		return myMatrix[currCol+(currRow*cols)];

		// if ((currRow>=rows) || (currCol>=cols) || (currRow<0) || (currCol<0)) return -1;
		// else return myMatrix[currCol+(currRow*cols)];
	};
	/**
	 ...
	 */
	 int setElement(int currRow, int currCol, int value){
		...
		if ((currRow>=rows) || (currCol>=cols) || (currRow<0) || (currCol<0)){
			...
			myMatrix[currCol+(currRow*cols)]=value;
			return 1;
		}
